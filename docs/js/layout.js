...
function prepareSearchField() {
	$('#search').on('click', '#search-submit', function(e) {
		e.preventDefault()
		$('#search').submit()
	})
	...
	$('#search').on('submit', function(e) {
		...
		var input = $('#search-field')[0].value
		var site = 'https://chain.com' + window.location.pathname.split('/', 3).join('/')
		var query = input + ' site:' + site
